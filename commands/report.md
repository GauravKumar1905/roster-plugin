...
1. `list_accounts` — find the account id for the named client
2. `describe_account` — see what the account actually supports before designing anything
3. `get_catalog` — use only metric and dimension ids that exist; inventing one is the most
   common way a saved report fails validation
4. `save_report_template` with `applyToAccountId` — save and render in one step
